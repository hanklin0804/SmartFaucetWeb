...
mkdir resources # 放config.properties 檔案
...
brokerUrl=tcp://192.53.162.144:1883
clientId=JavaMqttClient
EOF
...
public class MqttClientDemo {
    ...
    public static void main(String[] args) {
        ...
        try {
            PrintStream fileOut = new PrintStream("./logs/MqttClientDemo.log");
            System.setOut(fileOut);
            System.setErr(fileOut);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
            System.exit(1); // Exit if the log file cannot be opened
        }
        ...
        try {
            ...
            e.printStackTrace();
            System.exit(1); // Exit if the config file cannot be read
        }

        String brokerUrl = properties.getProperty("brokerUrl"); // Default value if not found in config
        String clientId = properties.getProperty("clientId"); // Default value if not found in config
